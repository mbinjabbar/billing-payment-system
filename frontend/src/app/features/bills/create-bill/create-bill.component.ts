...
export class CreateBillComponent {
  ...
  get filteredInsuranceFirms(): any[] {
    ...
    const type = visit.data.appointment.patient_case.car_accident
      ? 'auto'
      : 'health';

    const search = this.insuranceSearch().toLowerCase();

    return this.insuranceFirms().data.filter((f: any) =>
      f.firm_type?.toLowerCase() === type &&
      (f.name?.toLowerCase().includes(search) ||
       f.firm_type?.toLowerCase().includes(search))
    );
  }
  ...
  getSelectedInsuranceName(): string {
    return this.insuranceFirms().data.find(
  ...
  summary = computed(() => {
    ...
    const total = this.selectedProcedures().reduce(
      (sum, p) => sum + Number(p.standard_charge),
      0
    );
  ...
  onSubmit() {
    ...
    this.billService.createBill(this.buildPayload('Pending')).subscribe({
      next: (res: any) => {
        this.router.navigate(['bills/invoice', res.data.id]);
      },
      error: (err) => {
