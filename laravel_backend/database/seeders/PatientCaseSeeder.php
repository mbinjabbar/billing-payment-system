    ...
    public function run(): void
    {
        ...
        for ($i = 1; $i <= 25; $i++) {
            PatientCase::create([
                ...
                'case_category' => $faker->randomElement(['Cardiology', 'Orthopedics', 'General Medicine', 'Neurology', 'Pediatrics', 'Dermatology', 'Gynecology', 'Ophthalmology', 'ENT', 'Dental', 'Psychiatry']),
                'priority'      => $faker->randomElement(['Low', 'Normal', 'High', 'Urgent']),
                'status'        => $faker->randomElement(['Active', 'Closed', 'Transferred', 'On Hold']),
                'description'   => $faker->paragraph(1),
                'opened_date'   => $faker->dateTimeBetween('-1 month', 'now'),
