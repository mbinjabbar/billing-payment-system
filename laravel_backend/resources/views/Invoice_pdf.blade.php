...
<body style="font-family: Arial, sans-serif; font-size: 12px; color: #111; margin:0; padding:0; line-height:1.4;">
    ...
    <!-- Container -->
    <div
        style="max-width: 800px; margin: 20px auto; border:1px solid #ccc; border-radius:8px; overflow:hidden; padding:20px;">

        <!-- Header -->
        <!-- Header -->
        <div style="text-align:center; margin-bottom:20px;">
            <h1 style="margin:0; font-size:24px; font-weight:bold;">Medical Invoice</h1>
            <p style="margin:4px 0 0 0; font-size:12px; color:#555;">{{ $settings['clinic_name'] ?? 'MedBilling' }}</p>
            <p style="margin:2px 0 0 0; font-size:11px; color:#555;">{{ $settings['clinic_address'] ?? '' }}</p>
            <p style="margin:2px 0 0 0; font-size:11px; color:#555;">{{ $settings['clinic_phone'] ?? '' }} |
                    ...
                    @foreach($bill->procedure_codes ?? [] as $code)
                        <tr>
                            ...
                            <td style="border:1px solid #ccc; padding:6px; text-align:right;">
                                ${{ number_format($code['standard_charge'] ?? 0, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align:right;">
                            -${{ number_format(($bill->charges ?? 0) * ($bill->insurance_coverage / 100), 2) }}</td>
                    </tr>
                    <tr>
                        <td>Discount</td>
